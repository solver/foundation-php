...
class RadarStrategy {
	...
	public function __construct($cacheDir, array $symbolDirs = null) {
	...
	public function find($symbolId) {
		...
		if ($didMap) {
			if (isset($this->map[$symbolId])) {
				return $this->base . $this->map[$symbolId];
			} else {
				return null;
			}
		} else {
			...
			$import = function ($path, $symbolId) {
				...
				if (isset($this->map[$symbolId])) {
					...
					if ($this->map[$symbolId] !== $path) {
						throw new \Exception('Symbol "' . $symbolId . '" has been declared twice, at "' . $path . '" and "' . $this->map[$symbolId] . '".');
					}
				} else {
